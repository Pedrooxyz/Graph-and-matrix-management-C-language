...
Grafo* EliminaVertice(Grafo* grafo, int codVertice, bool* res) {
	*res = false;
	...
	EliminaTodasAdj(aux->pAdj, res); //Como as Adj estão "ligadas" ao vértice e já não é preciso esse espaço ocupado
...
bool BuscaProfundidade(Grafo* grafo, Vertice* vertice, Vertice* destino, int* caminho, int pos) {
	...
	if (vertice == destino) {
		...
		ExportarCaminhoParaFicheiro(caminho, pos);// Exporta o caminho para um ficheiro
	}
	...
	while (adjacente != NULL) {
		if (!adjacente->visitado) {
			...
			if (vizinho != NULL && !vizinho->visitado) {
				...
				if (BuscaProfundidade(grafo, vizinho, destino, caminho, pos + 1)) {
					return true;
				}
			}
		}
